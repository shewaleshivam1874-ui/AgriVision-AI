...
def gemini_result():
    ...
    preview_img = session.get('gemini_preview_img') or session.get('preview_img') or session.get('image_path')
    pred_id = session.get('gemini_pred_id')

    if not preview_img and pred_id:
        try:
            rec = db_manager.get_prediction_by_id(pred_id)
            if rec and rec.get('image_path'):
                preview_img = rec['image_path']
        except Exception:
            pass
