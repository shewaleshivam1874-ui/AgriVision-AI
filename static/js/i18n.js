/**
 * AgriVision AI - Multilingual Internationalization Engine (English | मराठी | हिंदी)
 * Provides instant client-side localization across all static and dynamic components.
 */
...
const translations = {
  en: {
    ...
    // Report Page (/result & /gemini-result)
  ...
  hi: {
    ...
    tech_opencv_title: "ओपनसीवी और पिलो (OpenCV & Pillow)",
    tech_vision_desc: "फ़ाइल सत्यापन, स्केलिंग और पत्ती की गुणवत्ता जांच के लिए कंप्यूटर विज़न टूल।",
    tech_gradcam_title: "ग्रैड-कैम एक्सएआई (Grad-CAM XAI)",
...
// =====================================================================
// DYNAMIC LOOKUP DICTIONARIES FOR CLIENT-SIDE LOCALIZATION
// =====================================================================
...
const healthStatusMap = {
  ...
  'environmental_stress': { mr: '🌡️ वातावरणाचा ताण', hi: '🌡️ वातावरणीय तनाव', en: '🌡️ ENVIRONMENTAL STRESS' },
  'unknown': { mr: '⚪ अज्ञात स्थिती', hi: '⚪ अज्ञात स्थिति', en: '⚪ UNKNOWN' }
};

const severityMap = {
  'healthy': { mr: '🟢 निरोगी पान (Healthy)', hi: '🟢 स्वस्थ पत्ती (Healthy)', en: '🟢 Healthy Leaf' },
  ...
  'early': { mr: '🟡 सुरुवातीचा टप्पा (Early)', hi: '🟡 प्रारंभिक चरण (Early)', en: '🟡 Early Stage' },
  'early stage': { mr: '🟡 सुरुवातीचा टप्पा', hi: '🟡 प्रारंभिक चरण', en: '🟡 Early Stage' },
  'moderate': { mr: '🟠 मध्यम टप्पा (Moderate)', hi: '🟠 मध्यम चरण (Moderate)', en: '🟠 Moderate Stage' },
  'moderate stage': { mr: '🟠 मध्यम टप्पा', hi: '🟠 मध्यम चरण', en: '🟠 Moderate Stage' },
  'developing': { mr: '🟠 वाढणारा टप्पा', hi: '🟠 विकासशील चरण', en: '🟠 Developing' },
  'advanced': { mr: '🔴 प्रगत टप्पा', hi: '🔴 उन्नत चरण', en: '🔴 Advanced Stage' },
  'advanced stage': { mr: '🔴 प्रगत टप्पा', hi: '🔴 उन्नत चरण', en: '🔴 Advanced Stage' },
  'severe': { mr: '🔴 गंभीर टप्पा (Severe)', hi: '🔴 गंभीर चरण (Severe)', en: '🔴 Severe Stage' },
  ...
  'unknown': { mr: '⚪ अज्ञात तीव्रता', hi: '⚪ अज्ञात गंभीरता', en: '⚪ Unknown Severity' }
};
...
function translateCrop(name, lang = 'en') {
  if (!name) return name;
  const l = (lang || 'en').toLowerCase();
  if (l === 'en') return name;
  const k = String(name).trim().toLowerCase();
  if (cropNameMap[k]) return cropNameMap[k][l] || name;
  for (let key in cropNameMap) {
    if (k.includes(key)) return cropNameMap[key][l] || name;
  }
  return name;
}

function translateDisease(name, lang = 'en') {
  if (!name) return name;
  const l = (lang || 'en').toLowerCase();
  if (l === 'en') return name;
  const k = String(name).trim().toLowerCase();
  if (diseaseNameMap[k]) return diseaseNameMap[k][l] || name;
  for (let key in diseaseNameMap) {
    if (k.includes(key) || key.includes(k)) return diseaseNameMap[key][l] || name;
  }
  return name;
}

function translateCategory(cat, lang = 'en') {
  if (!cat) return cat;
  const l = (lang || 'en').toLowerCase();
  if (l === 'en') return cat;
  const k = String(cat).trim().toLowerCase();
  if (categoryMap[k]) return categoryMap[k][l] || cat;
  for (let key in categoryMap) {
    if (k.includes(key)) return categoryMap[key][l] || cat;
  }
  return cat;
}

function translateStatus(st, lang = 'en') {
  if (!st) return st;
  const l = (lang || 'en').toLowerCase();
  if (l === 'en') return st;
  const k = String(st).trim().toLowerCase();
  if (healthStatusMap[k]) return healthStatusMap[k][l] || st;
  for (let key in healthStatusMap) {
    if (k.includes(key)) return healthStatusMap[key][l] || st;
  }
  return st;
}

function translateSeverity(sev, lang = 'en') {
  if (!sev) return sev;
  const l = (lang || 'en').toLowerCase();
  if (l === 'en') return sev;
  const k = String(sev).trim().toLowerCase();
  if (severityMap[k]) return severityMap[k][l] || sev;
  for (let key in severityMap) {
    if (k.includes(key)) return severityMap[key][l] || sev;
  }
  return sev;
}

function translateUrgency(urg, lang = 'en') {
  if (!urg) return urg;
  const l = (lang || 'en').toLowerCase();
  if (l === 'en') return urg;
  const k = String(urg).trim().toLowerCase();
  if (urgencyMap[k]) return urgencyMap[k][l] || urg;
  for (let key in urgencyMap) {
    if (k.includes(key)) return urgencyMap[key][l] || urg;
  }
  return urg;
}
...
function setLanguage(lang) {
  ...
  // 1. Update text of elements with data-i18n attribute
  ...
  // 4. Update dropdown selector state
  ...
  document.querySelectorAll('.disease-crop-tag').forEach(tag => {
    ...
    tag.textContent = '🌱 ' + translateCrop(tag.getAttribute('data-raw-crop'), lang);
  });
  ...
  document.querySelectorAll('.dynamic-category').forEach(el => {
    const orig = el.getAttribute('data-raw') || el.textContent.replace(/^Category:\s*/i, '').trim();
    if (!el.getAttribute('data-raw')) el.setAttribute('data-raw', orig);
  ...
  try {
    fetch('/set-language/' + encodeURIComponent(lang) + '?json=1', {
      method: 'POST',
      headers: {
  ...
  // 7. Dispatch custom DOM event for other scripts
...
document.addEventListener('DOMContentLoaded', () => {
  const savedLang = localStorage.getItem('agrivision_lang') || document.documentElement.getAttribute('lang') || 'en';
